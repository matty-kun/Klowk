...
import { Activity, useTracking, Category } from '@/context/TrackingContext';
...
import { ScrollView, TextInput, TouchableOpacity, View, Text, Pressable, Animated, Easing } from 'react-native';
...
export default React.memo(function LogsScreen() {
  const { activities, deleteActivity, duplicateActivity, categories } = useTracking();
  const { t } = useLanguage();
  ...
  const openFilter = () => {
    ...
    Animated.spring(filterAnim, { toValue: 1, useNativeDriver: true, tension: 60, friction: 10 }).start();
  };

  const closeFilter = () => {
    Animated.timing(filterAnim, { toValue: 0, duration: 200, easing: Easing.out(Easing.ease), useNativeDriver: true }).start(() => setShowFilter(false));
  };
  ...
  const filtered = React.useMemo(() => {
    return activities.filter((a: Activity) => {
      const matchesSearch = search.trim()
        ...
        : true;
      const matchesCat = selectedCategories.length === 0 || selectedCategories.includes(a.category || '');
    ...
    <SafeAreaView className="flex-1 bg-white dark:bg-klowk-black" edges={['top']}>
      <ScrollView className="flex-1 px-6" showsVerticalScrollIndicator={false}>
        ...
        <View className="flex-row items-center justify-between mb-6 mt-8">
          <View className="flex-row items-center gap-3">
            ...
            <Text className="text-4xl font-black text-klowk-black dark:text-white">{t('history')}</Text>
          </View>
          <Pressable
            onPress={openFilter}
            style={{
              width: 44, height: 44,
              borderRadius: 22,
              backgroundColor: activeFilterCount > 0 ? '#FF5A00' : (isDark ? '#27272a' : '#f3f4f6'),
              alignItems: 'center',
              justifyContent: 'center',
            }}
          >
            <SlidersHorizontal size={18} color={activeFilterCount > 0 ? '#fff' : (isDark ? '#9ca3af' : '#6b7280')} />
            {activeFilterCount > 0 && (
              <View style={{
                position: 'absolute', top: -2, right: -2,
                width: 16, height: 16, borderRadius: 8,
                backgroundColor: '#fff', alignItems: 'center', justifyContent: 'center',
                borderWidth: 1.5, borderColor: '#FF5A00'
              }}>
                <Text style={{ fontSize: 8, fontWeight: '900', color: '#FF5A00' }}>{activeFilterCount}</Text>
              </View>
            )}
          </Pressable>
        </View>
            ...
            {selectedCategories.map(id => {
              ...
              return (
                <Pressable
                  ...
                  style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 20, backgroundColor: `${cat.color}20`, borderWidth: 1, borderColor: cat.color }}
                >
                  <CategoryIcon name={cat.iconName} size={11} color={cat.color} />
                  <Text style={{ marginLeft: 6, fontSize: 11, fontWeight: '700', color: cat.color }}>{t(cat.id as any) || cat.label}</Text>
        ...
        {Object.entries(grouped).length === 0 && (search.trim() || activeFilterCount > 0) ? (
          <View style={{ alignItems: 'center', paddingVertical: 48 }}>
            <Text style={{ color: '#d1d5db', fontWeight: '700', fontSize: 14 }}>
              {search.trim() ? `${t('no_logs')} "${search}"` : 'No logs match the selected filters.'}
            </Text>
          </View>
        ) : null}
      ...
      {showFilter && (
        <>
          ...
          >
            {/* Sheet handle */}
            <View style={{ width: 36, height: 4, borderRadius: 2, backgroundColor: isDark ? '#3f3f46' : '#e5e7eb', alignSelf: 'center', marginBottom: 24 }} />

            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
              <Text style={{ fontSize: 20, fontWeight: '900', color: isDark ? '#fff' : '#121212' }}>Filter</Text>
              {activeFilterCount > 0 && (
                <Pressable onPress={clearFilters}>
                  <Text style={{ fontSize: 13, fontWeight: '700', color: '#FF5A00' }}>Clear all</Text>
                </Pressable>
              )}
            </View>

            <Text style={{ fontSize: 10, fontWeight: '900', color: isDark ? '#71717a' : '#9ca3af', textTransform: 'uppercase', letterSpacing: 2, marginBottom: 14 }}>
              {t('category_label')}
            </Text>
                ...
                return (
                  <Pressable
                    ...
                    style={{
                      ...
                      borderRadius: 16,
                      backgroundColor: isSelected ? `${cat.color}18` : (isDark ? '#27272a' : '#f9fafb'),
                      borderWidth: 1.5,
                      borderColor: isSelected ? cat.color : 'transparent',
                    }}
                  >
                    <CategoryIcon name={cat.iconName} size={13} color={isSelected ? cat.color : (isDark ? '#71717a' : '#9ca3af')} />
                    <Text style={{ marginLeft: 8, fontSize: 13, fontWeight: '700', color: isSelected ? cat.color : (isDark ? '#a1a1aa' : '#6b7280') }}>
                      {t(cat.id as any) || cat.label}
                    </Text>
                    {isSelected && (
                      <View style={{ marginLeft: 8, width: 16, height: 16, borderRadius: 8, backgroundColor: cat.color, alignItems: 'center', justifyContent: 'center' }}>
            ...
            <Pressable
              onPress={closeFilter}
              style={{
                marginTop: 28, paddingVertical: 16, borderRadius: 20,
                backgroundColor: isDark ? '#fff' : '#121212',
                alignItems: 'center'
              }}
            >
              <Text style={{ fontWeight: '900', fontSize: 14, color: isDark ? '#121212' : '#fff', textTransform: 'uppercase', letterSpacing: 1 }}>
                Done
              </Text>
            </Pressable>
          </Animated.View>
